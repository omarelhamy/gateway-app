...
COMMENT ON TABLE gateways IS 'IoT gateway devices that manage peripheral devices';
COMMENT ON TABLE peripheral_devices IS 'Peripheral devices that can be attached to gateways';
COMMENT ON TABLE device_types IS 'Types of peripheral devices';
COMMENT ON TABLE gateway_logs IS 'Audit log of all gateway operations';

COMMENT ON COLUMN gateways.serial_number IS 'Unique serial number for the gateway';
COMMENT ON COLUMN gateways.ipv4_address IS 'IPv4 address of the gateway';
COMMENT ON COLUMN gateways.status IS 'Current operational status of the gateway';

COMMENT ON COLUMN peripheral_devices.uid IS 'Globally unique identifier for the device';
COMMENT ON COLUMN peripheral_devices.gateway_id IS 'Reference to the gateway this device is attached to (NULL if orphaned)';
COMMENT ON COLUMN peripheral_devices.last_seen_at IS 'Last time the device was seen online';
